...
df_pred = pd.read_csv("predictions_NB_GaussianHMM.csv")  # Predictions from Hybrid NB + Gaussian HMM model

# Ensure correct column names exist
if "NB_PredictedDelay" not in df_pred.columns or "Hybrid_PredictedDelay" not in df_pred.columns:
    raise ValueError("Error: Missing required columns ('NB_PredictedDelay' or 'Hybrid_PredictedDelay') in predictions file.")

y_true = df_true['Delay'].values  # Actual delays
y_pred_nb = df_pred['NB_PredictedDelay'].values  # Naïve Bayes predictions
y_pred_hybrid = df_pred['Hybrid_PredictedDelay'].values  # Hybrid (NB + Gaussian HMM) predictions
...
print("\nMODEL PERFORMANCE EVALUATION (Naïve Bayes vs Hybrid NB + Gaussian HMM)")
...
# Print Hybrid NB + Gaussian HMM metrics
print_metrics("Hybrid NB + Gaussian HMM", metrics_hybrid)

print("\nInsights & Analysis:")
if metrics_hybrid["accuracy"] > metrics_nb["accuracy"]:
    print(f"  The Hybrid NB + Gaussian HMM model outperforms Naïve Bayes, improving accuracy from {metrics_nb['accuracy']*100:.2f}% to {metrics_hybrid['accuracy']*100:.2f}%.")
if metrics_hybrid["recall"] > metrics_nb["recall"]:
    print(f"  The Hybrid model catches more actual delays (Recall: {metrics_nb['recall']*100:.2f}% → {metrics_hybrid['recall']*100:.2f}%).")
...
plot_confusion_matrix(y_true, y_pred_hybrid, "Hybrid NB & Gaussian HMM - Normalized Confusion Matrix", "confusion_matrix_hmm.png")