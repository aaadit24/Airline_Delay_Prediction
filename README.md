...
Through this model, we have estimated the probability of a flight delay based on different features (**Airline, Airport, Time, etc.**) using the following formula:

\[
P(Delay | Features) = \frac{P(Features | Delay) P(Delay)}{P(Features)}
\]

This allows the model to determine whether a flight is likely to be **delayed** or **on time**.
...
In **train_NB.py**, we dynamically compute conditional probabilities instead of relying on precomputed Conditional Probability Tables (CPTs).  
This improves the **model’s flexibility** and allows it to **adapt to different datasets**.
...
| **File** | **Description** |
|----------|----------------|
| `Airlines.csv` | Original Dataset taken from Kaggle |
| `airlines_preprocessing.py` | Cleaned and preprocessed the raw dataset |
| `preprocessed_airlines.csv` | Processed dataset used for modeling |
| `train_NB.py` | Naïve Bayes training and prediction |
| `predictions_NB.csv` | Dataset with model predictions (Delayed/Not Delayed) |
| `evaluate_NB.py` | Code to evaluate the model and compute accuracy, precision, recall, and F1-score |
